...
The pipeline is triggered when a new commit or a merge request to the master branch is made.

It may seem that the pipeline is very simple, but it is what we aimed for. Nevertheless, the last part namely the "Upload" part is the more complex, since it does all the heavy lifting updating the right codes and updating all the environment libraries to always have everything up to date.

The pipeline steps are divided into 3 phases:
...
The 'Upload' phase checks if changes were made to the content of any lambda function. If changes were detected, the new lambda content is zipped again and the terraform plan will be modified.
More over also the libraries are checked for any incoming changes and uploaded accordingly. That does not mean though that the libraries.zip file is created every time on the action, since it would take a lot of time. Instead the method that we used, is, since the libraries are rarely updated, we create the zip file locally and check for changes in it in the github action, with the same filtering approach as before.
Once a change is found, the pipeline uploads the file and therefore a new layer version is created, so we had to add 2 lines, that will update the version of the layer in the lambda function configuration.
We did that leveraging Amazon's CLI commands such as
...
Another time consuming part was the integration of Amazon CLI which provided poor documentation, and at first also difficult to integrate in github actions. Thanks to the amazing community online we were able to find examples to make everything run smooth.
...
I only deployed one of the two functions, so to try, because if I were to upload the "updating message" function it would have had a conflict with the one on AWS. But I found that migrating one, was the hardest part, migrating two or more wouldn't be exponentially more difficult than just from 0 to 1.
...
I also draw some conclusion which I will explain also in class during the PPT presentation on the 11/01/2022.