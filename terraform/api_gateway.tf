...
resource "null_resource" "set_webhook_telegram" {
  ...
  provisioner "local-exec" {
    command = "curl https://api.telegram.org/${telegram_token}/setWebhook?url=${aws_apigatewayv2_stage.default.invoke_url}/${aws_lambda_function.fiemmebot_modify_data.function_name}"
  }
}
