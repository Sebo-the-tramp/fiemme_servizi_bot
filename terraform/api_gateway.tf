...
resource "null_resource" "set_webhook_telegram" {
  ...
  provisioner "local-exec" {
    command = "curl https://api.telegram.org/${var.telegram_token}/setWebhook?url=${var.aws_apigatewayv2_stage.default.invoke_url}/${aws_lambda_function.fiemmebot_modify_data.function_name}"
  }
}
