...
def get_text(comune):    
    ...
    if(list_mondizie):
        ...
        text += "Promemoria!\n"    
        text += "Domani mattina verra effettuata la raccolta dei seguenti rifiuti:"
        
        if("VETRO" in list_mondizie.split(",") and len(list_mondizie.split(",")) > 1):                            
            for mondizia in list_mondizie.split(","):      
                print(mondizia)
                if("VETRO" in mondizia):            
                    ...
                    is_vetro_week = ((day_month-1)//7+1)%2 != VETRO[comune]
                    if(is_vetro_week):
                        text += "\n- VETRO 🍾"
                    break            
